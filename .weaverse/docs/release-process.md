...
# 4. Regenerate the component manifest (it stamps the theme version)
npm run weaverse:manifest

# 5. Commit, tag, and push via PR (main is protected)
git add package.json package-lock.json .weaverse/component-manifest.json
...
Step 4 is equally required. The manifest records the theme version, so a bump
alone puts it out of date and CI fails the release PR.
