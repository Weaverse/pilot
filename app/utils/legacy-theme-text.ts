 ...
 * 2. The legacy theme setting, while it still holds a value — copy from before
 *    the migration, which must never be silently discarded.
...
export function legacyThemeText(
  ...
  const legacy = legacyName ? settings?.[legacyName] : undefined;
