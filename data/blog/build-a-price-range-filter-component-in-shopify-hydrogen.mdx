...
```ts title="($locale).collection.$collectionHandle.tsx" showLineNumbers
...
```tsx title="price-range-filter.tsx" showLineNumbers
...
```ts title="price-range-filter.tsx" showLineNumbers
import type { ProductFilter } from "@shopify/hydrogen/storefront-api-types"
import type { CollectionQuery } from "storefront-api.generated"
