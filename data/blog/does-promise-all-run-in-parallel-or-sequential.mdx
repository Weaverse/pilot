...
```js title="promises.js" showLineNumbers
...
```js title="promise-hell.js" showLineNumbers
promise1().then(() => {
  promise2().then(() => {
    promise3().then(() => {
      // do other stuff
...
```js title="concurrently.js" showLineNumbers
await Promise.all([promise1(), promise2(), promise3()])
// or
await Promise.all(
...
```js title="sequentially.js" showLineNumbers
