...
````ts title="use-route-submission.ts" showLineNumbers
...
```ts title="object.ts" showLineNumbers
...
```ts title="use-index-route-detector.ts"
...
```ts title="hooks.ts" showLineNumbers
