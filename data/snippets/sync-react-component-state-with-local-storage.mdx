...
```ts title="use-local-storage-state.ts" showLineNumbers
...
```ts title="use-local-storage-state.ts" showLineNumbers {3,7-12}
import { useState, useEffect } from 'react'
...
```tsx title="theme-switcher.tsx" showLineNumbers
